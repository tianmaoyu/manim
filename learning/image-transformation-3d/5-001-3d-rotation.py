...
class ThreeDRotation00y(ThreeDScene):
    def construct(self):
        ...
        animate = axes.animate.rotate(PI / 2, RIGHT)
        self.play(animate)
        animate = axes.animate.rotate(PI / 2, OUT)
        self.play(animate)
        #
        # animate = axes.animate.rotate(PI / 2, IN)
        # self.play(animate)

        self.move_camera(theta=10*DEGREES, phi=30*DEGREES)

#右手
class ThreeDRotation00y2(ThreeDScene):
    def construct(self):

        self.set_camera_orientation(phi=65 * DEGREES, theta=35 * DEGREES)

        axes = ThreeDAxes(include_numbers=False, x_range=[-4, 4, 1], y_range=[-4, 4, 1], z_range=[-4, 4, 1], x_length=8, y_length=8, z_length=8)
        axes.add(axes.get_axis_labels(x_label="x",y_label="z",z_label="y"))
        self.play(Create(axes))
...
class ThreeDRotation00y3(ThreeDScene):
    def construct(self):
        ...
        start_group = VGroup()
        start_group.add(number_plane,circle,start_dot,start_vector,start_arc,start_arc_label,start_lines,start_vector_label)
        start_group.rotate(90*DEGREES,axis=RIGHT)

        end_group = VGroup()
        end_group.add(end_dot, end_vector, end_arc, end_arc_label, end_lines,end_vector_label)
        end_group.rotate(90 * DEGREES, axis=RIGHT)
        self.wait()
